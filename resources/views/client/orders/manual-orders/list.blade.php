    ...
    <table class="table table-bordered">
        ...
        <tbody>  
            ...
            <tr style="background-color:@if($lists->status == 'deleted')#f99c9c @elseif($lists->status == 'prepaired') #b7b8b9 @elseif($lists->status == 'confirmed') #91c6ff @elseif($lists->status == 'dispatched') #84f39c @elseif($lists->status == 'deleted') #e77272 @else #f9df90 @endif">
                ...
                ?>
                <th><a target="_blank" href="https://api.whatsapp.com/send?phone=<?=$reciever_number?>&text=Hi{{$lists->first_name}},please click on the link to and check your articles and press confirmed button {{route('ManualOrders.confirm.order.by.customer.show',$lists->id)}}"><?=$reciever_number?></a></th> 
                <th><a target="_blank" href="https://api.whatsapp.com/send?phone=<?=$number?>&text=Hi{{$lists->first_name}},please click on the link to and check your articles and press confirmed button {{route('ManualOrders.confirm.order.by.customer.show',$lists->id)}}"><?=$number?></a></th> 
